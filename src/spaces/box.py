...
class Box(Space):
    """A (possibly unbounded) box in R^n. Specifically,
    a Box represents the Cartesian product of n closed intervals.
    Each interval has the form of one of
    ...
    * Independent bound for each dimension::
    ...
    def __init__(self, low, high, shape=None, dtype=np.float32):
        ...
        if shape is not None:
            shape = tuple(shape)
            assert np.isscalar(low) or low.shape == shape, (
                "low.shape doesn't match provided shape")
            assert np.isscalar(high) or high.shape == shape, (
        ...
        # Boolean arrays which indicate the interval type
        # for each coordinate
    ...
    def sample(self):
        ...
        * (-oo, oo) : normal distribution
        """
        high = self.high if self.dtype.kind == 'f' else (
            self.high.astype('int64') + 1)
    ...
    def __eq__(self, other):
        return isinstance(other, Box) and (self.shape == other.shape) and (
